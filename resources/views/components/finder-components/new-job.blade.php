@props(['$musicianNumber', '$payment', '$musician'])
    ...
    <div class="bg-white px-4 py-5 sm:p-6">
        <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-3 sm:col-span-2 lg:col-span-1">
                ...
                <input id="payment-for-job" value="{{ old("musicians.$musicianNumber.payment") ?? ($payment ?? '') }}" name="musicians[{{ $musicianNumber }}][payment]" type="number" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
            <div class="col-span-6 lg:col-span-3">
                <label for="extra_info" class="block text-sm font-medium text-gray-700">Extra Details</label>
                <input type="text" name="musicians[{{ $musicianNumber }}][extra_info]" value="{{ old("musicians.$musicianNumber.extra_info") }}" id="extra_info" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
        </div>
    </div>
</div>
