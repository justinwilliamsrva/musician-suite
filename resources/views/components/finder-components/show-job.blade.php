        ...
        <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-2 lg:col-span-1">
                <label for="musician_name" class="block text-sm font-medium text-gray-700">
                    Musician #{{ $job['musicianNumber'] ?? $musicianNumber }}
                </label>
                <!--
                    @if(count(array_intersect($user['instruments'], $job['instruments'])) > 0)
                        Add button to Apply to job
                    @else
                        SHOW name of person playing, pending, or open
                    @endif
                -->
            </div>
